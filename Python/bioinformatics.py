...
def prokka_search(directory, search_term):
    ...
    outputs:
        match_list = list of matched files
        line_list = list of matched lines
    also included extra lines for parsing into dataframe file, couldnt fit well into function
    ...
    return match_list, line_list

    result = prokka_search("./mag annotation/prokka/tsv/*.tsv","Cyanophycin")

#######
# include with prokka_search!!!
for i in range(0,len(result[0])):
    result[0][i] = result[0][i].replace("./mag annotation/prokka/tsv\\","")
    result[0][i] = result[0][i].replace(".tsv","")
    result[1][i] = result[1][i].replace("\n","")

result_df = pd.DataFrame([x.split("\t") for x in result[1]], columns=["locus_tag","ftype","length_bp","gene","EC_number","COG","product"])
#######
